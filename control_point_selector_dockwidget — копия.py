    ...
    def getWeatherDataFromOWM(self, lat, lon):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.label_temperature_OWM.setText(f"Температура: {temperature}°C")
                self.label_windSpeed_OWM.setText(f"Скорость ветра: {wind_speed} м/с")
                self.label_windDirection_OWM.setText(f"Направление ветра: {wind_direction}")
                ...
                self.clearWeatherInfo()
                self.label_temperature_OWM.setText("Ошибка получения данных о погоде.")
    ...
    def getControlPointsForObject(self):
        ...
        for feature in features:
            ...
            if self.current_wind_direction in allowed_wind_directions:
                suitable_control_points.append(feature.id())

        self.label_controlPoints.setText(f"Подходящие точки контроля: {', '.join(map(str, suitable_control_points))}")
